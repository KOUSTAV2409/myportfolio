    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {FEATURED_CONTENT.map((content, index) => (
          <InteractiveCard key={content.id} className="h-full">
            <motion.div
              className="h-full p-6 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors cursor-pointer"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
      ...
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-xl p-6 border border-indigo-200/50 dark:border-indigo-800/50 text-center"
      >
        <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
          Never Miss a Technical Insight
        </h4>
        <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">
          Join 5,000+ developers getting weekly deep dives into scaling challenges
        </p>
        <div className="flex items-center gap-3 max-w-md mx-auto">
          <input
            type="email"
            placeholder="your@email.com"
            className="flex-1 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <button className="px-6 py-2 brand-gradient text-white rounded-lg font-medium text-sm hover:opacity-90 transition-opacity">
            Subscribe
          </button>
        </div>
